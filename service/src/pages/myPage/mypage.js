  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const recipeResponse = await axios.get(mockApiUrl);

        const userData = { userName: '마피' };
    ...
    <div className="my-page">
      ...
      <MyPageContainer>
        {myRecipes.map(recipe => (
          <MyPageBox key={recipe.id}>
            <ProfileDetails>
              <BoxProfile src={recipe.profileImage} alt="Profile" />
              <span style={{padding : '0px 9px'}}>{recipe.userName}님</span>
            </ProfileDetails>
      ...
      <MyPageContainer>
        {myPicks.map(pick => (
          <MyPageBox key={pick.id}>
            <ProfileDetails>
              <BoxProfile src={pick.profileImage} alt="Profile" />
              <span style={{padding : '0px 9px'}}>{pick.userName}</span>
            </ProfileDetails>
